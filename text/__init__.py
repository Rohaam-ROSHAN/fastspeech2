...
from text.symbols import symbols, out_symbols, list_pct_with_no_space_after, list_pct_with_no_space_before
import numpy as np
...
def _find_pattern_indexes_in_batch(list_patterns, texts):
    ...
    for patterns_data in list_patterns:
        ...
        for i_batch, text in enumerate(texts):
            start_pattern_indexes = [x+index_in_pattern for x in range(len(text)-len(index_pattern_in_symbols)+1) if np.all(text[x:x+len(index_pattern_in_symbols)].cpu().numpy() == index_pattern_in_symbols)]
