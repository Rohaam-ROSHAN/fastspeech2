This README comes from the [original Repository](https://github.com/ming024/FastSpeech2) and is not up to date.

# Modifications from the original repository
- Visual Decoder
- Phonetic Prediction from Encoder's output
- Training on Mixed Batches (full, only audio, only text...)
- Global Style Tokens Supported (control by reference not yet supported)
- BERT embeddings Supported
- Local Style Tokens Supported
...
```
python3 synthesize.py --source preprocessed_data/LJSpeech/val.txt --restore_step 90000 --mode batch -p config/LJSpeech/preprocess.yaml -m config/LJSpeech/model.yaml -t config/LJSpeech/train.yaml
```
