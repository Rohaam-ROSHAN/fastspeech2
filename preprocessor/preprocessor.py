...
class Preprocessor:
    def __init__(self, config):
        ...
        self.sampling_rate_au = config["preprocessing"]["au"]["sampling_rate"]
    ...
    def build_from_path(self):
        ...
        # os.makedirs((os.path.join(self.out_dir, "durationau")), exist_ok=True)
        ...
        for i, speaker in enumerate(tqdm(os.listdir(self.in_dir))):
            ...
            for wav_name in os.listdir(os.path.join(self.in_dir, speaker)):
                ...
                # dur_au_filename = "{}-durationau-{}.npy".format(speaker, basename)
                ...
                if os.path.exists(tg_path):
                    ...
                    if ret is None:
                        ...
                        print(info)
                    out.append(info)
#     I should change, I think  these two 'if' statements and the stetement after those(lines 113-120, next 8 lines) shoud be indented in previous else clause
                if len(pitch) > 0:
    ...
    def process_utterance(self, speaker, basename, trim_silence):
        ...
        if mel_spectrogram.shape[1] < sum(duration) : 
            malmel += 1
            # print('this is mel size difference : ',  mel_spectrogram.shape[1] - sum(duration) , 'malmel: ', malmel)
        mel_spectrogram = mel_spectrogram[:, : sum(duration)] # RR, what if the length is already shorter than sum(duration)?
        ...
        # np.save(os.path.join(self.out_dir, "durationau", dur_au_filename), duration_au)
    ...
    def get_alignment(self, tier, trim_silence):
        ...
        for t in tier._objects:
            ...
            # durations_au.append(
            #     int(
            #         np.round(e * self.sampling_rate_au )
            #         - np.round(s * self.sampling_rate_au )
            #     )
            # )
        ...
        if trim_silence:
            ...
            # durations_au = durations_au[:end_idx]
