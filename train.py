...
def main(args, configs):
    ...
    # Prepare model
    model, optimizer, flaubert, flaubert_tokenizer = get_model(args, configs, device, train=True, use_bert=model_config["bert"]["use_bert"])# RR, use bert is false now
    ...
    # Get dataset
    dataset = Dataset(
        train_config["path"]["train_csv_path"], preprocess_config, train_config, sort=True, drop_last=True, use_bert=model_config["bert"]["use_bert"], flaubert=flaubert, flaubert_tokenizer=flaubert_tokenizer
    )
        ...
        for batchs in loader:
            for batch in batchs:
                ...
                if step % log_step == 0:
                    ...
                    message2 = "Total Loss: {:.4f}, Mel Loss: {:.4f}, Mel PostNet Loss: {:.4f}, Pitch Loss: {:.4f}, Energy Loss: {:.4f}, Duration Loss: {:.4f}, Phon Loss: {:.4f}, Visual Loss: {:.4f}, Visual Postnet Loss: {:.4f}, Style Loss: {:.4f}".format(
                        *losses
                    )
                ...
                if step % val_step == 0:
                    model.eval()
                    message = evaluate(model, step, configs, output_wav, val_logger, vocoder, no_spectro=args.no_spectro, flaubert=flaubert, flaubert_tokenizer=flaubert_tokenizer)
